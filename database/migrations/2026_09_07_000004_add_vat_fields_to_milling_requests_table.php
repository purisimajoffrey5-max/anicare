...
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach ($columns as $name => $addColumn) {
            if (!Schema::hasColumn('milling_requests', $name)) {
                Schema::table('milling_requests', function (Blueprint $table) use ($addColumn) {
                    $addColumn($table);
                });
            }
        }
    ...
    {
        foreach ([
            'vat_enabled', 'vat_rate', 'vatable_sales', 'vat_amount', 'total_sales',
        ] as $column) {
