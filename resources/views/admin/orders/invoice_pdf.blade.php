...
<style>
    ...
    .header .title{font-size:22px;font-weight:bold;margin:3px 0}
    .muted{color:#666}
    .meta{width:100%;border-collapse:collapse;margin-bottom:14px}
    .meta td{width:50%;background:#f5f7f6;padding:9px;border:1px solid #e6ebe8}
    .parties{width:100%;border-collapse:separate;border-spacing:8px 0;margin:0 -8px 16px}
    .parties td{width:50%;vertical-align:top;border:1px solid #e3e8e5;padding:10px}
    .party-title{color:#198754;font-weight:bold;margin-bottom:6px}
    table.items{width:100%;border-collapse:collapse;margin-top:12px}
    .items th{background:#f1f4f2;text-align:left;padding:8px;border:1px solid #dfe5e2}
    .items td{padding:8px;border:1px solid #e3e8e5;vertical-align:top}
...
<table class="meta">
    <tr>
        <td>
            <span class="muted">Order Number</span><br>
...
<table class="parties">
    <tr>
        <td>
            <div class="party-title">BUYER / ADMIN</div>
            <strong>{{ $invoice['buyer_name'] }}</strong><br>
            {{ $invoice['buyer_contact'] }}<br>
            {{ $invoice['buyer_address'] }}
        </td>

        <td>
            <div class="party-title">SELLER / FARMER</div>
            <strong>{{ $invoice['farmer_name'] }}</strong><br>
            {{ $invoice['farmer_address'] }}
        </td>
    </tr>
</table>
...
<table class="items">
    ...
    <tbody>
        <tr>
            ...
            <td>
                PHP {{ number_format($invoice['unit_price'],2) }}
                {{ $invoice['unit_price_label'] }}
            </td>

            <td class="right">
                PHP {{ number_format($invoice['subtotal'],2) }}
            </td>
        </tr>
    </tbody>
</table>
...
<table class="totals">
    <tr>
        <td>Subtotal</td>
        <td class="right">
            <strong>PHP {{ number_format($invoice['subtotal'],2) }}</strong>
        </td>
    </tr>
...
<div class="note">
    This is an Order Invoice / Order Confirmation for transparency.
    It is not an official payment receipt while payment is unpaid.
</div>
